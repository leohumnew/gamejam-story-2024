      ...
      for(int j = 0; j < itemPositions[i].length; j++) {
        ...
        if(pos != -1) {
          ...
          if(interactables.containsKey(i) && (posX + width/2)/S > pos && (posX + width/2)/S < pos + staticItems[i].width) {
            tint(170);
            image(staticItems[i], pos*S - posX, height - staticItems[i].height*S - mainLayers[0].height*S * 0.8 + yOffset*S, staticItems[i].width*S, staticItems[i].height*S);
            image(ui[0], pos*S - posX + staticItems[i].width*S/2 - ui[0].width*S/2, max(height - (staticItems[i].height + mainLayers[0].height * 0.85 + ui[0].height + yOffset)*S, 12*S), ui[0].width*S, ui[0].height*S);
            noTint();
          } else image(staticItems[i], pos*S - posX, height - staticItems[i].height*S - mainLayers[0].height*S * 0.8 + yOffset*S, staticItems[i].width*S, staticItems[i].height*S);
        }
      }
    }
  }
